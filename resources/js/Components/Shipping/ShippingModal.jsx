...
import { FaTimes, FaSave, FaTruck, FaDollarSign, FaClock, FaFileAlt, FaSort } from 'react-icons/fa';
import { toast } from 'react-toastify';

const ShippingModal = ({ isOpen, onClose, shipping = null, isEdit = false }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const onSuccess = () => {
            toast.success(isEdit ? 'Méthode de livraison modifiée avec succès' : 'Méthode de livraison créée avec succès');
            reset();
            onClose();
        };
        ...
        if (isEdit) {
            patch(route('admin.shippings.update', shipping.id), {
                onSuccess,
                onError,
                onFinish
            });
                ...
                <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 rounded-t-lg">
                    <div className="flex justify-between items-center">
                        <h2 className="text-xl font-bold text-white flex items-center gap-2">
                            <FaTruck />
                            {isEdit ? 'Modifier la méthode de livraison' : 'Nouvelle méthode de livraison'}
                        </h2>
                        <button
                            onClick={handleClose}
                            ...
                            <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                                ...
                                <div className="text-right">
                                    <div className="font-bold text-green-600">
                                        {data.price ? `${parseFloat(data.price).toFixed(2)} €` : 'Gratuit'}
                                    </div>
                                    {data.estimated_days && (
