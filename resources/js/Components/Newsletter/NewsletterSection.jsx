        ...
        <section className={`py-12 ${variants[variant]} ${className}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-8">
                    <h2 className={`text-3xl font-bold ${variant === 'light' ? 'text-gray-900' : 'text-white'} mb-4`}>
                        {title}
                    </h2>
                    <p className={`text-lg ${variant === 'light' ? 'text-gray-600' : 'text-gray-200'} max-w-2xl mx-auto`}>
                        {subtitle}
                    </p>
                </div>
                ...
                <div className="text-center mt-8">
                    <div className="flex items-center justify-center space-x-4 text-sm">
                        <span className={`flex items-center ${variant === 'light' ? 'text-gray-500' : 'text-gray-300'}`}>
                            <EnvelopeIcon className="h-4 w-4 mr-1" />
                            Envoi hebdomadaire
                        </span>
                        <span className={`${variant === 'light' ? 'text-gray-300' : 'text-gray-500'}`}>•</span>
                        <span className={`flex items-center ${variant === 'light' ? 'text-gray-500' : 'text-gray-300'}`}>
                            <GiftIcon className="h-4 w-4 mr-1" />
                            Offres exclusives
                        </span>
                        <span className={`${variant === 'light' ? 'text-gray-300' : 'text-gray-500'}`}>•</span>
                        <span className={`${variant === 'light' ? 'text-gray-500' : 'text-gray-300'}`}>
